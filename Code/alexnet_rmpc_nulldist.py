...
def prepare_null_distribution_acc(alexnet, data, target, corr_num, pc=1, n_iter=1000):
    ...
    outdata_conv4 = outdata_conv4.detach().cpu()
    for n in range(n_iter):
        ...
        outdata_conv4_tmp = outdata_conv4_tmp.to('cuda:0')
        ...
        outdata_tmp_np = outdata_tmp.detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    alexnet.load_state_dict(torch.load('/nfs/a1/userhome/huangtaicheng/workingdir/models/DNNmodel_param/alexnet.pth', map_location='cuda:0'))
    ...
    dataloader = torch.utils.data.DataLoader(imagefolder, batch_size=1000, num_workers=10, shuffle=False)
    ...
    for i, (img, target) in enumerate(dataloader):
        print('Image {}:{}'.format(1000*i,1000*(i+1)))
        img = img.to('cuda:0')
        # actval_conv4_rmpc, actval_rmpc = reorganize_alexnet(alexnet, img, pcamodel=pcamodel, pc=4)
    ...
    np.save('alexnet_rmpc_nulldist_acc_ingpu.npy', acc_rdm)
